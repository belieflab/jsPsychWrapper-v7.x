    ...
    if (fieldId === "subject") {
        ...
        if (
            ...
            const regex = new RegExp(
                `^${intake.subject.prefix}\\d{${digitCount}}$`
            );
            if (!regex.test(element.value)) {
                validateAlerts.push(
                    `Subject ID does not meet format. Must begin with ${intake.subject.prefix} followed by ${digitCount} digits, totaling ${intake.subject.minLength} characters.`
                );
                return;
            }
        }
...
function validateIntake() {
    ...
    // ONLY ADD THE FOLLOWING NEW CODE - This runs only if validation passed
