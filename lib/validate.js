...
function validateField(fieldId, errorMessage) {
    ...
    if (fieldId === "subject") {
        element.value = element.value.toUpperCase();
        if (intake.subject.prefix && intake.subject.minLength) {
            ...
            if (!regex.test(element.value)) {
                validateAlerts.push(
                    `Subject ID does not meet format. Must begin with ${intake.subject.prefix} and be ${intake.subject.minLength} characters long.`
                );
                return;
            }
        }
