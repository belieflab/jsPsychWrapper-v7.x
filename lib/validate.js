...
function validateField(fieldId, errorMessage) {
    ...
    if (fieldId === "subject") {
        ...
        // Site-specific validation (keeping existing logic)
        if (site === "UCD") {
            const regex = /^C10D.{4}$/;
            if (!regex.test(element.value)) {
                addValidationAlert(
                    "Subject Id does not meet format. Must begin with C10D and be 8 characters long."
                );
                return null;
            }
        }
        if (site === "UMN") {
            ...
            if (!regex.test(element.value)) {
                addValidationAlert(
                    "Subject Id does not meet format. Must begin with C10M and be 8 characters long."
                );
                return null;
            }
        }
        if (site === "Maryland") {
            ...
            if (!regex.test(element.value)) {
                addValidationAlert(
                    "Subject Id does not meet format. Must begin with C10B and be 8 characters long."
                );
                return null;
            }
        }
        if (site === "UChicago") {
            ...
            if (!regex.test(element.value)) {
                addValidationAlert(
                    "Subject Id does not meet format. Must begin with C10C and be 8 characters long."
                );
                return null;
            }
        }
        if (site === "WashU") {
            ...
            if (!regex.test(element.value)) {
                addValidationAlert(
                    "Subject Id does not meet format. Must begin with C10W and be 8 characters long."
                );
                return null;
            }
        }
...
function validateIntake() {
    ...
    console.log("VALIDATION: Starting intake validation");

    // Check if already processing to prevent double submission
    if (window.validationInProgress) {
        console.log(
            "VALIDATION: Already in progress, ignoring duplicate submission"
        );
        return;
    }
    ...
    // Only validate GUID if NIH study
    ...
    // SMALL CHANGE: Only display validation errors if there are any
    if (window.validateAlerts && window.validateAlerts.length > 0) {
        window.validationInProgress = false; // Reset flag on validation failure
        displayValidationErrors();
        ...
        testDataSave()
            .then((result) => {
                if (result && result.success) {
                    ...
                    // **KEY CHANGE**: Prevent duplicate timeline loading
                    if (window.timelineLoaded) {
                        console.log(
                            "Timeline already loaded, skipping duplicate load"
                        );
                        return;
                    }
                    window.timelineLoaded = true;

                    // Load the experiment timeline AFTER validation
                    console.log(
                        "Loading timeline with visit:",
                        visit,
                        "week:",
                        week
                    );
                    ...
                    // Re-enable submit button and reset flags
