...
/**
 * Tests data saving functionality with automatic timestamp appending for existing files.
 * Checks if a file with the given name already exists, and if so,
 * appends a timestamp to the filename to avoid overwriting.
 *
 * @returns {Promise<Object>} Resolves to an object with success status and filename used
 */
async function testDataSave() {
    // Determine base filename based on available identifiers
    let baseFilename;
    ...
    try {
        ...
        // Define the actual filename to use
        ...
        // Proceed with test save using the determined filename
        const testData = "This is a test save";
        const response = await saveDataPromise(finalFilename, testData);

        console.log("Data saved successfully.", response);

        // Store the filename for future use
        window.successfulFilename = finalFilename;
...
function checkFileExistsPromise(filename) {
    return new Promise((resolve) => {
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState === 4) {
                console.log(
                    `File check for ${filename}.csv status: ${xhr.status}`
                );
                if (xhr.status === 200) {
        ...
        xhr.onerror = function () {
            console.log(`Error checking if file exists: ${filename}.csv`);
...
/**
 * Saves experiment data to CSV and handles redirect after completion.
 * Uses the successful filename from testDataSave if available.
 *
 * @async
 * @function
 */
...
async function writeCsvRedirect() {
    ...
    // Use the filename from testDataSave if available
    // This skips redundant file existence checks
    let filename = window.successfulFilename;

    if (!filename) {
        console.log("No pre-tested filename available, using default");
        // Default filename construction
        if (visit) {
            filename = `${experimentAlias}_${subjectId}_v${visit}`;
        } else if (week) {
            filename = `${experimentAlias}_${subjectId}_w${week}`;
        } else if (phase) {
            filename = `${experimentAlias}_${subjectId}_phase${phase}`;
        } else {
            filename = `${experimentAlias}_${subjectId}`;
        }

        // If no pre-tested save was done, append timestamp automatically
        // to avoid any potential conflicts
        const timestamp = generateTimestamp();
        filename = `${filename}_${timestamp}`;
    }

    // Save data using Promise pattern with the determined filename
    saveDataPromise(filename, jsPsych.data.get().csv())
        .then((response) => {
            // response will tell us if data has been saved on server
            console.log("Data saved successfully with filename:", filename);
