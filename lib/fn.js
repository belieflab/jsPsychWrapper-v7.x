...
document.addEventListener("DOMContentLoaded", (event) => {
    ...
    if (!location.search) {
        ...
        if (visitSelectElement) {
            intake.visits.forEach((visit) => {
                const visitOption = new Option(visit, visit);
                visitSelectElement.add(visitOption);
            });
        }

        const weekSelectElement = document.getElementById("week");
        if (weekSelectElement) {
            intake.weeks.forEach((week) => {
                const weekOption = new Option(week, week);
                weekSelectElement.add(weekOption);
            });
        }
    }
    ...
    if (intake.visits && intake.visits.length > 0) {
        const visitHtml = `
    ...
    if (intake.weeks && intake.weeks.length > 0) {
        const weekHtml = `
