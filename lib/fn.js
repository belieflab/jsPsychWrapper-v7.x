...
//     );
// }
// Function to determine the correct link path
function getLinkPath() {
    // Feature detection or a more robust method can be used here
    const isWindows = window.navigator.userAgent.indexOf("Windows") !== -1;
    return isWindows ? "wrap\\link\\data.php" : "./wrap/link/data.php";
}

function saveData(
    name = `${experimentAlias}_${subjectId}`,
    data = jsPsych.data.get().json(), // Ensure data is JSON
    callback
) {
    const xhr = new XMLHttpRequest();
    const linkPath = getLinkPath(); // Use dynamically determined path
    xhr.open("POST", linkPath);
    xhr.setRequestHeader("Content-Type", "application/json");
    xhr.onreadystatechange = () => {
        if (xhr.readyState === 4) {
