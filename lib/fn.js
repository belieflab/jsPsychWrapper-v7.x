...
/**
 * Loads language-specific content into the consent form on document ready.
 * This script waits until the DOM is fully loaded, then it constructs the path to the required language file
 * based on a global 'language' variable. It uses jQuery's AJAX method to fetch the language-specific content.
 * If the request is successful, the content is injected into the element with the id 'consentForm'.
 * In case of an error (e.g., file not found), it logs an error message to the console and could be extended
 * to handle failures more gracefully, such as loading a default language or showing a user-friendly error message.
 *
 * Ensure that the 'language' variable is defined and corresponds to an existing language file path.
 * Also, ensure jQuery is loaded before this script runs.
 *
 * This pattern is typically used in multi-language web applications to enhance user experience
 * by loading localized content without the need to reload the page.
 */
$(document).ready(() => {
