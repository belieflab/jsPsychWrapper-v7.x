 ...
 *
 * Parameters:
 *   keyCode1 - The key code for the first key to monitor.
 *   keyCode2 - The key code for the second key to monitor.
 *
 ...
 */
function buttonPressWithArguments(keyCode1, keyCode2) {
    ...
    if (tapTapElement) {
        ...
        const handleKeyPress = (keycode, isKeyDown) => {
            ...
            if (keyHeld1 || keyHeld2) {
                totalConfidence = moveConfidence(); // Function to update the confidence level
            }
        };
...
/**
 * Generates a feedback link for a survey based on the survey version and the participant's identifier.
 * This function constructs a URL that includes a query parameter indicating the type of identifier used.
 *
 * @param {string} version - The version of the survey, which determines the base URL.
 * @param {string} id - The unique identifier for the participant (e.g., workerId, PROLIFIC_PID, participantId).
 * @param {string} idType - The type of the identifier being used (e.g., 'workerId', 'PROLIFIC_PID', 'participantId').
 * @returns {string|undefined} A string containing the full URL for the feedback survey if both `id` and `idType` are provided;
 *                             otherwise, returns `undefined` if either `id` or `idType` is missing.
 * @example
 * // returns "https://yalesurvey.ca1.qualtrics.com/jfe/form/SV_8qsU4yfds5mH6Pc?workerId=12345"
 * getFeedbackLink("loss", "12345", "workerId");
 *
 * @example
 * // returns "https://yalesurvey.ca1.qualtrics.com/jfe/form/SV_bErtyAFIwnwDhWu?PROLIFIC_PID=abcde"
 * getFeedbackLink("default", "abcde", "PROLIFIC_PID");
 */
